...
df = sqlc.sql("""select
    ---a.operation_id, c.customer_id, mv.comments1, 
 `th`.`id` as `cd_id`,
    `c2`.`identification_number` as `cd_estab`,
    `a`.`operation_id` as `cd_cta_pagto`,
    --mv.comments1 as nr_ip,
 `i`.`nr_ip` as `nr_ip`,
    ---mv.event_date_time,
 date_Format(`mv`.`event_date_time`, 'yyyy/MM/dd') as `dt_saq_desp`,
    date_Format(`mv`.`event_date_time`, 'hh:mm:ss') as `hr_saq_desp`,
    `th`.`amount` as `vl_saq_desp_mn`,
    'BRL' as `cd_moeda`,
    `th`.`amount` as `vl_saq_desp_mo`,
    1 as `tp_registro`,
    `sb2`.`subproduct_id` as `tp_despesa`,
    `mv`.`balance_today` as `vl_saldo_reais`,
    `th`.`stamp_date_time`
from
...
inner join `activity` `act` on
    ( `tra`.`activity` = `act`.`id` )
inner join `activity_related` `acr` on
    ( `acr`.`activity` = `act`.`id` )
...
left join `subproduct` `sb2` on
    ...
    --left join veloe.identificador I on (I.idconta = a.operation_id and c.customer_id = I.idcliente  and I.placa = mv.comments1 
    --AND (  (mv.event_date_time BETWEEN  I.datainclusao AND I.dataatualizacao) or I.situacao = 'ATIVO'))
left join (
    SELECT
        `identificador`.`idconta`,
        `identificador`.`idcliente`,
        MAX(`identificador`.`nuidentificador`) as `nr_ip`
    FROM
        `identificador`
    group by
        `identificador`.`idconta`,
        `identificador`.`idcliente`) `I` on
    (`i`.`idconta` = `a`.`operation_id`
    and `c`.`customer_id` = `i`.`idcliente`)
where
    `iso`.`nemotecnico` <> 'COMER_EC'
    and `sb2`.`short_desc` <> 'RECARGA'
""")
...
dfp.to_sql(con= my_con, name = "saq_desp_aux", if_exists ="replace", index = False)