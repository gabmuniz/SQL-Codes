...
subproductRelationsSchema = StructType() \
      .add('id',StringType(),True) \
      .add('subproduct_orig',StringType(), True) \
      .add('product_orig',StringType(), True) \
      .add('subproduct_rel', StringType(), True) \
      .add('product_rel',StringType(),True) \
      .add('status',StringType(), True) \
      .add('status_date',StringType(), True) \
      .add('status_reason',StringType(), True) \
      .add('stamp_user',StringType(), True) \
...
team = sqlc.read.options(delimiter="\020").schema(teamSchema).csv("caminho da tabela")
subproduct_relations = sqlc.read.options(delimiter="\020").schema(subproductRelationsSchema).csv("caminho da tabela")
...
subproduct_relations.createOrReplaceTempView('subproduct_relations')
transaction_reason.createOrReplaceTempView('transaction_reason')

df = sqlc.sql("""select
    `te`.`nsu` as `cd_id`,
    `c`.`identification_number` as `cd_estab`,
    `te`.`repasse_date` as `dt_pagamento`,
    sum(`th`.`amount`) as `vl_pag_mn`,
    'brl' as `cd_moeda`,
    sum(`th`.`amount`) as `vl_pag_mo`,
    `sp`.`subproduct_id` as `cd_produto`,
    MAX(`th`.`stamp_date_time`) as `stamp_date_time`
from
    `team` `t`
...
inner join `customer_operation_type` `tp` on
    ( `co`.`type` = `tp`.`id` )
inner join `customer` `c` on
    ( `co`.`customer` = `c`.`id` )
inner join `subproduct_relations` `rl` on
    ( `a`.`product` = `rl`.`product_orig`
    and `a`.`subproduct` = `rl`.`subproduct_orig` )
inner join `subproduct` `sb` on
    ( `rl`.`subproduct_rel` = `sb`.`id` )
inner join `transaction_reason` `tr` on
    ( `th`.`transaction_reason` = `tr`.`id` )
where
    `t`.`nemotecnico` = 'REPASSE'
    and `tp`.`nemotecnico` = 'FIRST_HOLDER_PR'
    and `te`.`nsu` is not null
group by
    `te`.`nsu`,
...
dfp.to_sql(con= my_con, name = "estabelecimentos_pagtos_aux", if_exists ="replace", index = False)