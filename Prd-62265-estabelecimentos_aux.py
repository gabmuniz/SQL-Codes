...
df = sqlc.sql("""select
    DISTINCT `c`.`identification_number` AS `CD_ESTAB`,
    `c`.`name` AS `NM_ESTAB`,
    `ac`.`central_bank_activity_code_id` AS `DE_ATIVIDADE`,
    ...
    `ed`.`fixed_income` AS `VL_FATURAMENTO_MEDIO`,
...
dfp.to_sql(con= my_con, name = "estabelecimentos_aux", if_exists ="replace", index = False)