...
df = sqlc.sql("""select
    `th`.`id` as `cd_id`,
    `c`.`identification_number` as `cd_resp_carga`,
    `c`.`name` as `de_resp_carga`,
    `a`.`operation_id` as `cd_cta_pagto`,
    ---mv.comments1,
 `i`.`nr_ip` as `nr_ip`,
    `mv`.`event_date_time` as `dt_transacao`,
    `th`.`amount` as `vl_aporte_mn`,
    'BRL' as `cd_moeda_op`,
    `th`.`amount` as `vl_aporte_mo`,
    case
        `iso`.`nemotecnico` when 'CLIENTES_PRE' then 1
        when 'CLIENTES_POS' then 2
    end as `tp_registro`,
    case
        `txg`.`nemotecnico` when 'RECARGA' then 5
        when 'DEBITO' then 7
        when 'CREDITO' then 5
        else 0
    end as `tp_forma_aporte`,
    '' as `cd_cpf_cnpj_contra`,
    '' as `cd_nie_contra`,
    '' as `de_pais_nie_contra`,
    '' as `nm_titular_contra`,
    '' as `cd_banco_contra`,
    '' as `cd_agencia_contra`,
    '' as `cd_conta_contra`,
    `th`.`stamp_date_time`
from
...
left join (
    ...
    where
        ...
        and `txg`.`nemotecnico` in ('RECARGA', 'DEBITO', 'CREDITO')
""")
...
dfp.to_sql(con= my_con, name = "pagtos_aportes_aux", if_exists ="replace", index = False)