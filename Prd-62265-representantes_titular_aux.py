...
df = sqlc.sql("""select
    ...
    concat('+55' , `cp`.`area_code` , `cp`.`phone_number`) as `cd_fone`,
    `co`.`participation_percentage` as `pc_participacao`,
    case
...
inner join `relation_customer_customer` `rcc` on
    (`rcc`.`customer_link` = `co`.`customer`)
inner join `relation_type` `rct` on
...
dfp.to_sql(con= my_con, name = "representantes_titular_aux", if_exists ="replace", index = False)